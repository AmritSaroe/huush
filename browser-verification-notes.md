...
## Completed exhaustive checks
Developer version seven-tap unlock, keyboard visualViewport behavior, accessibility focus order, diagnostics, and final console checks were completed in the prior pass. Reduced-motion behavior remains a physical/browser-preference verification caveat.
...
## 2026-08-26 Live Mint duplicate-image regression
The Android diagnostic log for `2.5.4-reader-fix-test` showed two image-load events for the Live Mint article within 1 ms. The URLs used different CDN date/size/logo/cache-token segments but shared the stable `FIGMA-STOCKS--0_1771842330210_1771842351498` asset identity. The previous normalizer only removed one size path segment, so both variants could survive. The Live Mint identity now derives a stable filename prefix plus the first two long numeric asset tokens, ignoring transformed date, size, logo, query, and cache-token segments. A focused Node test and the browser’s actual `stripImageByIdentity()` module both confirmed the source and transformed variants resolve to one identity and both inline duplicates are removed before the hero image is rendered.

## 2026-08-26 Reader top-edge clipping regression
The supplied Android screenshot exposed a reader chrome overlap that the prior pass missed: the top of a long-form reader surface could appear clipped beneath a horizontal toolbar edge, and full-root navigation snapshots could leave old/new article layers visually overlapping. Investigation found the final canonical-mobile `.reader-toolbar` rule restored `height/min-height/flex-basis:64px` after `.reader-toolbar.is-hidden`, so immersive mode retained a stale toolbar shell. The fix adds a final canonical hidden-toolbar override that collapses height, flex-basis, padding, visibility, and border. Reader entry/exit now bypasses the full-root View Transition snapshot because long-form article surfaces can overlap during that transition. Browser retest verified hidden mode at `toolbar height=0`, `surface top=0`, `surface height=1100`, `border=0`, with no stale top shell; fresh reader entry showed a complete toolbar and title at the top. Physical Android visual verification remains required.
...
## 2026-08-26 Temporary linked-article reader
A same-origin fixture link injected into the saved reader body was intercepted by the delegated click handler instead of opening the external browser. Huush opened a temporary reader with `Back to previous article`, a top `Save article to library` action, and a footer Save action. The temporary article did not change the Library count when Back was used; the parent article was restored. Tapping the top Save action persisted the linked article, changed the toolbar to the normal saved state/Organize action, and the Library count increased from 1 to 2 only after explicit consent. The feature uses the existing extraction and sanitization pipeline, preserves safe HTTP(S) links, and retains an Open in browser fallback for extraction errors.

## 2026-08-26 Reader toolbar layering regression
A browser reproduction using the canonical mobile surface confirmed the risk behind the Android screenshot: the reader toolbar and scroll surface shared a flex layout, while the scrolling article could paint toward the toolbar boundary during scroll and the final cascade mixed visible and immersive states. The focused fix places the reader in an explicit two-row grid, isolates the reader stacking context, gives the toolbar z-index 10 with an opaque background, and keeps the scroll surface in a separate z-index 0 row. Browser verification after the patch: visible state had toolbar `top=0,bottom=64,height=64`, surface `top=64,height=1036`, and `overlap=false`; hidden state had toolbar `height=0,visibility=hidden`, surface `top=0,height=1100`, and `overlap=false`. Physical Android WebView verification remains required.
